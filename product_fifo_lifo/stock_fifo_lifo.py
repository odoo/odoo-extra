...
class stock_mrp_matching(osv.Model):
    _name = 'stock.mrp.matching'
    
    _columns = {
        'move_in_id': fields.many2one('stock.move', 'Stock move in'),
        'move_out_id': fields.many2one('stock.move', 'Stock move out'),
        'mrp_qty': fields.float('Quantity in Uom of manufacture'),
        'price_unit_mrp': fields.float('Price in UoM of manufactured'),
        'product_id': fields.many2one('product.product', 'Product'),
        'date': fields.related('move_in_id','date', type="datetime", string="Date"),
    }
# vim:expandtab:smartindent:tabstop=4:softtabstop=4:shiftwidth=4:
